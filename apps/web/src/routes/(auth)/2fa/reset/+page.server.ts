import { getRecoveryCodeBucket, resetUser2FAWithRecoveryCode } from '$lib/server/auth/2fa';
...
async function action(event: RequestEvent) {
	...
	const recoveryCodeBucket = await getRecoveryCodeBucket(event.locals.user.id);
